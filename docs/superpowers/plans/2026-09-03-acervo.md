...
### Task 17: `acervo.csv` e `relatorio.json`

O CSV é o que dá sentido à pasta depois: sem ele o usuário tem 500 arquivos e
nenhuma métrica. O relatório é onde as falhas viram informação em vez de sumiço
silencioso.
...
- Produces:
  ...
  - `paraCsv(colunas, linhas) => string`
  - `csvDoAcervo(registros) => string` onde cada registro é
    `{ post, posicao, arquivos }`
  - `relatorioJson({ perfil, ordenacao, filtro, partes, falhas, comMarcaDagua }) => string`

**Detalhe que parece bobo e não é:** o arquivo sai com BOM UTF-8. Sem ele, o
Excel em português abre "São Paulo" como "SÃ£o Paulo" e o usuário conclui que a
...
```bash
npx vitest run test/csv.test.js
```

Esperado: `Failed to resolve import "../src/core/csv.js"`.
...
```bash
git add src/core/csv.js test/csv.test.js
git commit -m "feat: geracao do acervo.csv e do relatorio.json"
```
...
Junta todas as peças do núcleo num pipeline só: particiona, batiza, baixa em
paralelo, escreve o ZIP em streaming, marca o que saiu e registra o que falhou.

**Files:**
- Create: `src/acervo/baixador.js`
- Test: `test/baixador.test.js`

**Interfaces:**
- Consumes: `particionar`, `montarNome`, `larguraSeq`,
  `criarResolvedorDeColisao`, `pipelineOrdenado`, `escreverZip`,
  `csvDoAcervo`, `relatorioJson`
- Produces:
  - `criarBaixador({ adaptador, buscarMidia, abrirDestino, escreverTexto, repo, aoProgresso? })`
  - `baixador.baixar(args) => Promise<{ partes, falhas, baixados, arquivos }>`
    onde `args` é `{ jobId, perfil, profileKey, posts, posicoes, opcoes,
    template?, tamanhoParte?, concorrencia?, sinal? }`
  - `buscarMidia(url, sinal) => Promise<Blob>`
  - `abrirDestino(nome) => Promise<WritableStream>`
  - `escreverTexto(nome, texto) => Promise<void>`

**Decisões que os testes travam:**

1. Um post só é marcado como baixado quando **todas** as mídias selecionadas
   dele deram certo. Marcar um post pela metade faria a opção "pular já
   baixados" esconder um post incompleto para sempre.
2. O resolvedor de colisão é **por parte**, não global: dois ZIPs diferentes
   podem ter arquivos de mesmo nome sem problema, e reiniciar a contagem
   mantém os nomes previsíveis.
3. Uma mídia morta vira linha no `relatorio.json` e o lote continua.
4. `acervo.csv` e `relatorio.json` são escritos **depois** de todas as partes,
   com o resultado real — não com a intenção.

- [ ] **Step 1: Escrever o teste que falha**

```javascript
// test/baixador.test.js
import { describe, it, expect, vi } from "vitest";
...
describe("baixar", () => {
  ...
  it("numera os arquivos pela posição, com zeros à esquerda", async () => {
    const env = ambiente();
    const posts = criarPosts(3);
    await env.baixador.baixar(pedido(posts, { posicoes: posicoesDe(posts, 500) }));
    const csv = env.textos.get("acervo.csv");
    expect(csv).toContain("0500_");
  });
  ...
  it("marca como baixado só o post cujas mídias todas deram certo", async () => {
    ...
    expect(env.marcados.map((m) => m.key)).toEqual(["ig:@fulano#BOM"]);
    expect(r.falhas).toHaveLength(1);
  ...
  it("escreve o relatorio.json com as partes e as falhas", async () => {
    ...
    expect(rel.perfil).toBe("ig:@fulano");
  });
  ...
  it("registra no relatório o que saiu com marca d'água", async () => {
    ...
    const rel = JSON.parse(env.textos.get("relatorio.json"));
    expect(rel.comMarcaDagua).toEqual(["ig:@fulano#TT"]);
  });
...
```bash
npx vitest run test/baixador.test.js
```

Esperado: `Failed to resolve import "../src/acervo/baixador.js"`.

- [ ] **Step 3: Implementar**

```javascript
// src/acervo/baixador.js
import { particionar, TAMANHO_PADRAO_PARTE } from "../core/partition.js";
import { montarNome, larguraSeq, criarResolvedorDeColisao, TEMPLATE_PADRAO }
  from "../core/naming.js";
...
export function criarBaixador({
  ...
  async function baixar({
    ...
    await repo.jobs.salvar({
      jobId, profileKey, status: "concluido",
      partes: resumoPartes, concluidoEm: Date.now(),
    });
...
```bash
npx vitest run test/baixador.test.js && npm test
```

Esperado: 16 testes novos passando e a suíte inteira verde.

- [ ] **Step 5: Commitar**

```bash
git add src/acervo/baixador.js test/baixador.test.js
git commit -m "feat: baixador com particionamento, zip em streaming e relatorio"
```

---

**Marco: toda a lógica do produto está pronta e testada em Node.** O que falta é
interface e fiação: o botão, o modal, a grade e a aba que liga tudo.

> **Para as tarefas de interface (19 a 21):** invoque a skill
> `frontend-design:frontend-design` antes de escrever o CSS. O plano fixa
> estrutura, comportamento e testes; a direção visual é decisão da
> implementação, e essa skill existe para que ela não saia com cara de
> template. As únicas restrições visuais que o plano impõe estão nos testes:
> Shadow DOM obrigatório, identidade distinta por plataforma, e nada de texto
> em inglês na interface.

---

### Task 19: Content script — carregador, navegação SPA e botão flutuante

**Restrição que molda esta task:** um content script declarado no manifest é
**script clássico** e não aceita `import`. A saída é um carregador de três
linhas que faz `import()` dinâmico do módulo de verdade, que é web-accessible.

O outro detalhe traiçoeiro: as duas plataformas são SPA. Ir de um perfil para
um post e voltar **não recarrega a página**, então o botão precisa reagir a
`pushState` e `popstate` — e o Chrome não emite evento para `pushState`.

**Files:**
- Modify: `src/content/boot.js`
- Create: `src/content/app.js`
- Create: `src/content/button.js`
- Test: `test/dom/navegacao.test.js`
- Test: `test/dom/button.test.js`

**Interfaces:**
- Consumes: `adaptadorDaUrl`, `chaveDePerfil`
- Produces:
  - `observarNavegacao(aoMudar) => () => void` (devolve o cancelador)
  - `montarBotao({ adaptador, handle, aoClicar }) => HTMLElement`
  - `desmontarBotao()`
  - `iniciar()` em `app.js`
...
```bash
npx vitest run test/dom/navegacao.test.js test/dom/button.test.js
```

Esperado: falha ao resolver `src/content/app.js` e `src/content/button.js`.
...
const PALETA = {
  instagram: {
    fundo: "linear-gradient(135deg, #833ab4, #fd1d1d 55%, #fcb045)",
    texto: "#fff",
    sombra: "0 8px 24px rgba(214, 41, 118, .38)",
  },
  tiktok: {
    fundo: "#111",
    texto: "#fff",
...
export function montarBotao({ adaptador, handle, aoClicar }) {
  ...
  const cores = PALETA[adaptador.id];
  ...
  estilo.textContent = `
    ...
    button {
      ...
      background: ${cores.fundo}; color: ${cores.texto}; box-shadow: ${cores.sombra};
      ${cores.detalhe ? `outline: 1px solid ${cores.detalhe};` : ""}
      font: inherit; transition: transform .12s ease;
    }
    button:hover { transform: translateY(-1px); }
    button:focus-visible { outline: 2px solid #fff; outline-offset: 2px; }
    .arroba { opacity: .85; font-weight: 400; }
  `;
  ...
  botao.innerHTML = `<span>Acervo</span><span class="arroba">@${handle}</span>`;
...
- [ ] **Step 4: Implementar a observação de navegação e o app**
...
export function iniciar() {
  observarNavegacao((url) => {
    ...
    montarBotao({
      ...
      handle,
      aoClicar: () =>
        abrirModal({ adaptador, handle, profileKey: chaveDePerfil(adaptador, handle) }),
    });
  });
}
```
...
Esperado: 6 + 10 testes passando. O import de `./modal.js` em `app.js` ainda vai
falhar — crie um esboço agora e implemente na Task 20:
...
```bash
git add src/content test/dom/navegacao.test.js test/dom/button.test.js
git commit -m "feat: content script com deteccao de navegacao spa e botao flutuante"
```
...
A tela da referência, mais o que ela não tem: ordenação por relevância e o
estado do catálogo.

**A regra de honestidade da interface:** ordenar por curtidas ou views exige
catálogo completo. Em vez de deixar o usuário escolher e entregar um resultado
errado em silêncio, essas opções ficam desabilitadas **com o motivo escrito na
tela** e um atalho para indexar.

**Files:**
- Modify: `src/content/modal.js`
- Create: `src/content/modal.css.js` (o CSS como string, para entrar no Shadow DOM)
- Test: `test/dom/modal.test.js`

**Interfaces:**
- Consumes: `ORDENACOES` de `src/core/selection.js`
- Produces:
  - `ID_MODAL`
  - `estadoInicial() => Estado` com
    `{ filtro:"ambos", incluirCapaReel:false, modo:"faixa", de:1, ate:100, ordenacao:"sequencia", pularBaixados:true }`
  - `podeOrdenarPorRelevancia(catalogo) => boolean`
  - `rotuloDoBotao(estado, catalogo) => string`
  - `abrirModal({ adaptador, handle, profileKey, carregarCatalogo, aoConfirmar, aoIndexar, aoAbrirAcervo }) => Promise<HTMLElement>`
  - `fecharModal()`

- [ ] **Step 1: Escrever o teste que falha**

```javascript
// test/dom/modal.test.js
import { describe, it, expect, vi, beforeEach } from "vitest";
import {
...
```bash
npx vitest run test/dom/modal.test.js
```

Esperado: `estadoInicial is not a function`.
...
import { CSS_MODAL } from "./modal.css.js";
...
export async function abrirModal({
  ...
  const estilo = document.createElement("style");
  estilo.textContent = CSS_MODAL[adaptador.id];
    ...
    <div class="painel" role="dialog" aria-modal="true" aria-label="Acervo de @${handle}">
      ...
      <p class="catalogo">
        ${numero(catalogo.totalIndexado)} publicações no catálogo${
          catalogo.completo ? "" : " — catálogo incompleto"
        }
      </p>
      ...
      <section>
        <h2>Tipo de mídia</h2>
        <div class="opcoes">
          ${["ambos", "fotos", "videos"].map((valor) => `
            <label>
              <input type="radio" name="filtro" value="${valor}" ${
                valor === estado.filtro ? "checked" : ""
              } />
              <span>${{ ambos: "Fotos e vídeos", fotos: "Só fotos", videos: "Só vídeos" }[valor]}</span>
            </label>`).join("")}
        </div>
        <label class="linha">
      ...
      <section>
        <h2>O que baixar</h2>
        <div class="opcoes">
          ${["faixa", "manual", "tudo"].map((valor) => `
            <label>
              <input type="radio" name="modo" value="${valor}" ${
                valor === estado.modo ? "checked" : ""
              } />
              <span>${{ faixa: "Por faixa", manual: "Marcar uma a uma", tudo: "Tudo" }[valor]}</span>
            </label>`).join("")}
        </div>
      ...
      <section>
        ...
        <select name="ordenacao">
          ${ORDENACOES.map((valor) => `
            <option value="${valor}" ${
              ORDENACOES_DE_RELEVANCIA.has(valor) && !relevanciaLiberada ? "disabled" : ""
            }>${ROTULO_ORDENACAO[valor]}</option>`).join("")}
        </select>
        ${relevanciaLiberada ? "" : `
          <p class="aviso">
            Para ordenar por curtidas ou visualizações é preciso indexar o perfil
            inteiro antes — as métricas não aparecem na grade.
          </p>`}
        <label class="linha">
  ...
  const redesenhar = () => {
    camposFaixa.hidden = estado.modo !== "faixa";
...
Crie `src/content/modal.css.js` exportando `CSS_MODAL` com uma string por
plataforma (`instagram` e `tiktok`), seguindo a direção visual que a skill
`frontend-design:frontend-design` indicar. Os únicos requisitos que os testes
impõem: `:host { all: initial }` para isolar do CSS do site, `.campos-faixa`
respeitando o atributo `hidden`, e contraste suficiente em ambos os temas das
plataformas.

- [ ] **Step 5: Ligar o modal ao app**

Em `src/content/app.js`, o `aoClicar` do botão passa a fornecer as quatro
dependências, todas conversando com o service worker:
...
aoClicar: () =>
  abrirModal({
    ...
    aoIndexar: (args) =>
      chrome.runtime.sendMessage({ tipo: "abrirAcervo", perfil: args.profileKey, acao: "indexar" }),
    aoAbrirAcervo: (args) =>
      chrome.runtime.sendMessage({ tipo: "abrirAcervo", perfil: args.profileKey }),
    aoConfirmar: (pedido) =>
      chrome.runtime.sendMessage({ tipo: "abrirAcervo", perfil: pedido.profileKey, acao: "baixar", pedido }),
  }),
```
...
```bash
git add src/content/modal.js src/content/modal.css.js src/content/app.js test/dom/modal.test.js
git commit -m "feat: modal de selecao com ordenacao por relevancia"
```

---

### Task 21: Grade virtualizada

Três mil miniaturas não cabem no DOM. Renderizar todas trava a aba por vários
segundos e come centenas de MB. A grade desenha só o que está na tela mais uma
margem, e reaproveita os nós ao rolar.

**Files:**
- Create: `src/acervo/grid.js`
- Test: `test/dom/grid.test.js`

**Interfaces:**
- Consumes: nada
- Produces:
  - `criarGrade({ container, alturaLinha, colunas, renderizar, aoMudarSelecao?, margem? }) => Grade`
  - `Grade.definirItens(itens)`, `Grade.selecionadas() => Set<string>`,
    `Grade.marcar(chave, ligado)`, `Grade.marcarFaixa(dePos, atePos)`,
    `Grade.limparSelecao()`, `Grade.aoClicarItem(chave, { shift })`,
    `Grade.destruir()`
  - Cada item é `{ chave, posicao, ...dados }`; `renderizar(item, elemento)`
    preenche o nó.

**Detalhe que os testes travam:** clicar com Shift marca a faixa entre a última
marcada e a atual — é o gesto que o usuário espera de qualquer grade, e sem ele
marcar 200 itens à mão é inviável.
...
```bash
npx vitest run test/dom/grid.test.js
```

Esperado: `Failed to resolve import "../../src/acervo/grid.js"`.
...
export function criarGrade({
  ...
  function marcar(chave, ligado) {
    ...
    const el = palco.querySelector(`[data-chave="${CSS.escape(chave)}"]`);
    el?.setAttribute("aria-selected", ligado ? "true" : "false");
    avisar();
  }
...
```bash
git add src/acervo/grid.js test/dom/grid.test.js
git commit -m "feat: grade virtualizada com selecao por faixa e shift-clique"
```

---

### Task 22: A ponte e o destino em disco

Duas peças de fiação que ainda merecem teste próprio: o caminho que um request
de paginação percorre até o mundo MAIN e de volta, e a escrita no disco via
File System Access.

**O caminho completo de uma página:**

```
aba do Acervo → runtime.sendMessage → service worker → tabs.sendMessage
   → content script da aba do perfil → window.postMessage → hook (mundo MAIN)
   → fetch de verdade, na origem certa → postMessage de volta → ... → aba do Acervo
```

Parece muita volta, e é. Mas é o que garante que o request saia da origem do
Instagram, com os cookies e o contexto de navegação que o endpoint exige. Um
`fetch` disparado da aba do Acervo seria de outra origem e voltaria vazio ou
bloqueado.

**Files:**
- Create: `src/acervo/transporte.js`
- Create: `src/acervo/destino.js`
- Modify: `src/content/app.js` (a ponte)
- Test: `test/transporte.test.js`
- Test: `test/dom/ponte.test.js`

**Interfaces:**
- Produces:
  - `criarTransporteViaHook({ abaAlvo, enviar, tempoLimite? }) => (url, init) => Promise<{ok,status,json}>`
  - `instalarPonte({ chromeApi?, janela? }) => () => void` em `app.js`
  - `escolherPasta() => Promise<FileSystemDirectoryHandle>`
  - `criarDestinoEmPasta(pasta, segmentos) => Promise<{ abrirDestino, escreverTexto, caminho }>`
  - `criarDestinoEmMemoria({ baixar }) => { abrirDestino, escreverTexto, LIMITE_BYTES }`

- [ ] **Step 1: Escrever os testes que falham**

```javascript
// test/transporte.test.js
import { describe, it, expect, vi } from "vitest";
...
```bash
npx vitest run test/transporte.test.js test/dom/ponte.test.js
```

Esperado: falha ao resolver `src/acervo/transporte.js` e
`instalarPonte is not a function`.

- [ ] **Step 3: Implementar o transporte**

```javascript
// src/acervo/transporte.js
...
export function criarTransporteViaHook({ abaAlvo, enviar, tempoLimite = TEMPO_LIMITE_PADRAO }) {
  return async function transportar(url, init) {
    ...
    const estouro = new Promise((resolve) =>
      setTimeout(() => resolve({ ok: false, status: 0, json: null, erro: "tempo esgotado" }), tempoLimite),
    );

    try {
      const resposta = await Promise.race([enviar(pedido), estouro]);
      if (!resposta) return { ok: false, status: 0, json: null, erro: "sem resposta da aba do perfil" };
      return resposta;
    } catch (erro) {
...
- [ ] **Step 4: Implementar a ponte no content script**

Acrescentar a `src/content/app.js`:
...
export function instalarPonte({ chromeApi = chrome, janela = window } = {}) {
  ...
  const aoMensagemDaExtensao = (mensagem, _remetente, responder) => {
    ...
    janela.postMessage(
      { fonte: FONTE_CONTEUDO, tipo: "paginar", id: mensagem.id, url: mensagem.url, init: mensagem.init },
      "*",
    );
...
E `iniciar()` passa a chamar `instalarPonte()` antes de observar a navegação.

- [ ] **Step 5: Implementar o destino em disco**

```javascript
// src/acervo/destino.js
...
export const temFileSystemAccess = () => typeof window.showDirectoryPicker === "function";
...
```bash
npx vitest run test/transporte.test.js test/dom/ponte.test.js && npm test
```

Esperado: 5 + 4 testes novos passando e a suíte inteira verde.

- [ ] **Step 7: Commitar**

```bash
git add src/acervo/transporte.js src/acervo/destino.js src/content/app.js test/transporte.test.js test/dom/ponte.test.js
git commit -m "feat: ponte ate o hook e destino em disco via file system access"
```

---

### Task 23: A aba do Acervo

A tela onde tudo se junta. É o processo principal do sistema: comanda a
indexação, mostra o catálogo, aplica filtro e ordenação, e executa o download.

**Files:**
- Modify: `src/acervo/acervo.html`
- Create: `src/acervo/acervo.js`
- Create: `src/acervo/acervo.css`
- Modify: `src/background/roteador.js` (repassar a aba de origem)
- Test: `test/roteador-aba.test.js`

**Interfaces:**
- Consumes: praticamente tudo — `abrirAcervo` do db, `criarIndexador`,
  `criarBaixador`, `criarGrade`, `resolverSelecao`, `criarTransporteViaHook`,
  `criarDestinoEmPasta`, `adaptadorPorId`
- Produces: a página funcional. Sem interface nova para outros módulos.

**A mudança no roteador:** a aba do Acervo precisa saber **qual aba do perfil**
usar como origem dos requests. O service worker já conhece esse número (é
`remetente.tab.id`), então ele passa a repassá-lo na URL.

- [ ] **Step 1: Escrever o teste que falha**

```javascript
// test/roteador-aba.test.js
import { describe, it, expect, vi } from "vitest";
...
```bash
npx vitest run test/roteador-aba.test.js
```

Esperado: `aba` vem `null`, porque o roteador ainda não repassa a aba de origem.

- [ ] **Step 3: Ajustar o roteador**

No `case "abrirAcervo"` de `src/background/roteador.js`:
...
<html lang="pt-BR">
  ...
  <body>
    <header class="topo">
      <h1>Acervo</h1>
      <p class="perfil" id="perfil">—</p>
      <p class="estado" id="estado">Carregando…</p>
    </header>

    <section class="barra" aria-label="Filtros e ordenação">
      <label>Mídia
        <select id="filtro">
          <option value="ambos">Fotos e vídeos</option>
          <option value="fotos">Só fotos</option>
      ...
      <label>Ordenar por
        <select id="ordenacao">
          <option value="sequencia">Sequência do perfil</option>
      ...
      <label>Da <input type="number" id="de" min="1" value="1" /></label>
      <label>até <input type="number" id="ate" min="1" value="100" /></label>

      <label class="linha">
        <input type="checkbox" id="incluirCapaReel" /> Incluir capa dos vídeos
      </label>
      <label class="linha">
        <input type="checkbox" id="pularBaixados" checked /> Pular já baixados
      </label>

      <button id="indexar" type="button">Indexar perfil</button>
      <button id="escolherPasta" type="button">Escolher pasta</button>
      <button id="baixar" type="button" class="primario">Baixar</button>
      <button id="cancelar" type="button" hidden>Cancelar</button>
    </section>

    <main id="grade" class="grade" role="listbox" aria-label="Publicações"></main>

    <aside class="progresso" id="progresso" hidden>
      <h2>Download em andamento</h2>
      <p id="progressoTexto"></p>
      <ol id="partes"></ol>
    </aside>

    <script type="module" src="acervo.js"></script>
  </body>
</html>
```

- [ ] **Step 5: Escrever a fiação**

```javascript
// src/acervo/acervo.js
...
async function indexar() {
  ...
  try {
    ...
    dizer(`Indexação interrompida: ${erro.message}. Espere alguns minutos antes de tentar de novo.`);
  } finally {
    cancelador = null;
...
async function iniciar() {
  ...
  grade = criarGrade({
    ...
    aoMudarSelecao: (marcadas) => {
      $("baixar").textContent = marcadas.size > 0
        ? `Baixar ${numero(marcadas.size)} marcadas`
        : "Baixar";
    },
  });
  ...
  chrome.runtime.onMessage.addListener((mensagem) => {
    // A captura do hook traz a assinatura que a indexação vai reusar.
    if (mensagem?.tipo === "capturou") {
      repo.perfis.salvar({ key: profileKey, assinatura: mensagem.carga });
    }
  });
...
`acervo.css` fica a cargo da implementação, orientada pela skill
`frontend-design:frontend-design`. Os únicos requisitos estruturais: `#grade`
precisa de altura fixa e `overflow-y: auto` (a virtualização depende de
`clientHeight` e `scrollTop`), e `.item` precisa caber em `alturaLinha`.

- [ ] **Step 6: Rodar e ver passar**

```bash
npx vitest run test/roteador-aba.test.js && npm test
```

Esperado: 3 testes novos passando e a suíte inteira verde.

- [ ] **Step 7: Commitar**

```bash
git add src/acervo src/background/roteador.js test/roteador-aba.test.js
git commit -m "feat: aba do acervo ligando indexacao, catalogo e download"
```

---

### Task 24: README e verificação manual de ponta a ponta
...
contrato, não a realidade. Esta é a task que fecha essa lacuna.

**Files:**
- Create: `README.md`
- Create: `docs/verificacao-manual.md`
...
2. Clique no botão **Acervo** no canto inferior direito
3. **Indexar perfil inteiro** — leva alguns minutos num perfil grande e fica
   salvo para sempre. Ordenar por curtidas ou views exige esta etapa
4. Escolha mídia, ordenação e faixa, e clique em baixar
5. Na primeira vez, escolha a pasta de destino. A extensão lembra depois

## Custo

Zero, para sempre. Sem servidor, sem API paga, sem chave de nada. Tudo roda na
sua máquina, na sua sessão já logada.
...
- **Mudança de formato.** A extensão aprende a assinatura do request em tempo de
  execução, o que a torna resistente a mudanças. Uma reformulação profunda de
  payload ainda exigiria atualizar o adaptador.
...
`src/core/` não importa nada do navegador — é onde mora a lógica e é testável em
Node. `src/adapters/` isola o que é específico de cada plataforma.
...
- [ ] **Step 2: Escrever o roteiro de verificação manual**
...
Marque cada item. Um item que falha é bug, não "comportamento da plataforma".
...
- [ ] O visual do botão é distinto entre as duas plataformas
...
- [ ] **Step 3: Rodar a verificação manual inteira**

Este passo é o trabalho, não a papelada. Anote cada item que falhar e conserte
antes de seguir.

- [ ] **Step 4: Substituir as fixtures por capturas reais**

Com a extensão rodando num perfil real, abra o console da aba do perfil e salve
...
Anonimize handles e URLs, substitua os arquivos em `test/fixtures/` e rode
`npm test`. Se algum teste de parsing quebrar, a fixture escrita à mão divergia
do real — conserte o adaptador, não o teste.

- [ ] **Step 5: Commitar**

```bash
git add README.md docs/verificacao-manual.md test/fixtures
git commit -m "docs: readme e roteiro de verificacao manual de ponta a ponta"
```
...
Feita com os olhos frescos, comparando plano e spec seção por seção.

**Cobertura da spec.** Cada requisito tem tarefa: §4 arquitetura → Tasks 1, 14,
15, 19, 23; §5 coleta → Tasks 13, 14, 16; §6 modelo de dados → Task 7; §7 não
repetição → Tasks 7, 18; §8 partição e ZIP → Tasks 5, 8, 18, 22; §9
nomenclatura → Task 3; §10 adaptadores → Tasks 9 a 12; §11 interface → Tasks
19, 20, 21, 23; §12 resiliência → Tasks 6, 16, 18; §13 testes → todas, mais a
Task 24; §14 estrutura de arquivos → seguida; §15 riscos → registrados no README.

**Três desvios conscientes da spec, todos documentados na tarefa que os
introduz:**

1. `resolveMedia` da spec §10 virou `midiasParaBaixar`, devolvendo escolhas de
   mídia sem nome de arquivo (Task 11). O nome depende da posição no lote e do
   template, que o adaptador não conhece.
2. A spec §14 não previa `src/core/assinatura.js`, `src/core/csv.js`,
   `src/acervo/transporte.js` nem `src/acervo/destino.js`. Os quatro nasceram
   da restrição de que content script no mundo MAIN não aceita `import` e da
   necessidade de manter a lógica testável fora do navegador.
3. A spec §6 dizia que o campo da faixa recebia o `seq`; na verdade é a
   **posição** na lista filtrada e ordenada. A spec foi corrigida antes deste
   plano; é a distinção que torna útil ordenar por relevância.

**Consistência de tipos.** `Post`, `Midia`, `Assinatura` e `OpcoesMidia` vêm de
`src/core/tipos.js` (Task 2) e são usados com os mesmos nomes em todas as
tarefas. `midiasParaBaixar` tem a mesma assinatura nos dois adaptadores (Tasks
11 e 12). `transporte(url, init) => {ok, status, json}` é o mesmo contrato na
Task 16 e na Task 22. `abrirDestino(nome) => WritableStream` é o mesmo na Task
18 e na Task 22.

**Sem placeholders.** Nenhum "TBD" ou "implementar depois". Os dois pontos
deixados à implementação são explícitos e delimitados: o CSS do modal (Task 20)
e o CSS da aba (Task 23), ambos com os requisitos estruturais escritos e
apontando para a skill de design.

---

## Ordem de execução e marcos

| Marco | Tasks | O que você tem ao fim |
|---|---|---|
| Núcleo | 1–8 | `npm test` cobre toda a lógica, sem navegador |
| Adaptadores | 9–12 | Instagram e TikTok parseados e testados |
| Runtime | 13–16, 22 | Indexação funcionando de ponta a ponta |
| Lógica de download | 17–18 | Baixador completo, testado com dublês |
| Interface | 19–21, 23 | Extensão utilizável |
| Fechamento | 24 | Verificado contra as plataformas de verdade |

As Tasks 3 a 8 são independentes entre si e podem ser feitas em qualquer ordem
depois da Task 2. As Tasks 10 e 11 dependem da 9. Tudo o mais é sequencial.
